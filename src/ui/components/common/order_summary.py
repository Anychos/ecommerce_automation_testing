...
class OrderSummary(BaseComponent):
    ...
    def __init__(self, page: Page, root: Locator):
        ...
        self.delivery_info_text = self.delivery_info_container.get_by_test_id(
            "delivery-info-text"
        )

        self.order_item_row = self.root.locator("[data-testid^=order-item-row-]").first
        self.order_item_name = self.order_item_row.locator(
            "[data-testid^=order-item-name-]"
        )
        self.order_item_details = self.order_item_row.locator(
            "[data-testid^=order-item-details-]"
        )
        self.order_item_total_price = self.order_item_row.locator(
    ...
    @allure.step(
        "Проверка видимости элементов итоговой информации о заказе на странице {page_name}"
    )
    def check_visibility(
        self, *, page_name: str, is_free_delivery: bool = False
    ) -> None:
        """
        ...
        if page_name == "cart":
            ...
            expect(self.label_value("subtotal")).to_have_text(
                re.compile(r"\d+\.\d+\s₽")
            )
            ...
            if is_free_delivery:
                ...
                expect(self.delivery_info_text).to_have_text(
                    re.compile(
                        r"Добавьте товаров на \d+\.\d+\s₽ для бесплатной доставки"
                    )
                )
            else:
            ...
            expect(self.label_value("cart-items")).to_have_text(
                re.compile(r"\d+\sшт\.")
            )
            ...
            expect(self.order_item_details).to_have_text(
