...
from src.api.clients.product.client import (
    ProductAPIClient,
    get_private_product_client,
    get_public_product_client,
)
...
@pytest.fixture
def admin_private_product_client(
    admin: UserFixture,
) -> Generator[ProductAPIClient, None, None]:
    """
...
@pytest.fixture
def user_private_product_client(
    user: UserFixture,
) -> Generator[ProductAPIClient, None, None]:
    """
...
def create_product_factory(
...
) -> Generator[Callable[..., CreateProductFixture], None, None]:
    """
    ...
    def _create_product(**overrides) -> CreateProductFixture:
        """
        Создает продукт с указанными параметрами

        :return: Объект ProductFixture с информацией о продукте
        """
        request = CreateProductRequestSchema(**overrides)
    ...
    try:
        ...
        for product in created_products:
            try:
                ...
                cleanup_errors.append(
                    RuntimeError(
                        f"Не удалось удалить продукт {product.product_id}: {error}"
                    )
                )
...
def update_product_factory(
    ...
    def _update_product(
        ...
        **overrides,
        ...
        response = admin_private_product_client.full_update_product(
