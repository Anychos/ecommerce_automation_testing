...
def assert_product_in_order(
    *, actual: CreateOrderResponseSchema, cart: CartFixture, index: int
) -> None:
    """
    ...
    assert_field_value(
        item.line_total, product.request.price * cart.request.quantity, "line_total"
    )
...
def assert_create_order_response(
    *,
    ...
    assert_field_value(
        actual.items_total_amount, expected_items_total, "items_total_amount"
    )
    assert_field_exists(actual.delivery_fee_amount, "delivery_fee_amount")
    assert_field_value(
        actual.total_amount,
        actual.items_total_amount + actual.delivery_fee_amount,
        "total_amount",
    )
...
def assert_get_order_response(
    *, actual: GetOrderResponseSchema, expected: CreateOrderResponseSchema
) -> None:
    """
    ...
    assert_field_value(
        actual.items_total_amount, expected.items_total_amount, "items_total_amount"
    )
    assert_field_value(
        actual.delivery_fee_amount, expected.delivery_fee_amount, "delivery_fee_amount"
    )
    ...
    assert_field_value(
        actual.delivery_status, expected.delivery_status, "delivery_status"
    )
    ...
    for actual_item, expected_item in zip(actual.items, expected.items):
        assert_field_value(
            actual_item.product_id, expected_item.product_id, "product_id"
        )
        assert_field_value(
            actual_item.product_name, expected_item.product_name, "product_name"
        )
        assert_field_value(
        ...
        assert_field_value(
            actual_item.unit_price, expected_item.unit_price, "unit_price"
        )
        assert_field_value(actual_item.quantity, expected_item.quantity, "quantity")
        assert_field_value(
            actual_item.line_total, expected_item.line_total, "line_total"
        )
...
def assert_get_orders_response(
    *,
    ...
    orders_by_id = {order.id: order for order in get_orders_response}
...
def assert_unavailable_product_order_response(
