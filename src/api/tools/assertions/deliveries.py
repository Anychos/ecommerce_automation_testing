...
def _assert_quote_response(
    *, actual: QuoteBaseSchema, expected: QuoteRequestSchema
) -> None:
    ...
    assert_field_value(
        actual.dropoff_address, expected.dropoff_address, "dropoff_address"
    )
    assert_field_value(
        actual.dropoff_latitude, expected.dropoff_latitude, "dropoff_latitude"
    )
    assert_field_value(
        actual.dropoff_longitude, expected.dropoff_longitude, "dropoff_longitude"
    )
...
def assert_quote_delivery_response(
    *, actual: QuoteDeliveryResponseSchema, expected: QuoteRequestSchema
) -> None:
...
def _assert_delivery_entity(
    *,
    ...
    assert_field_value(
        actual.pickup_latitude, expected.pickup_latitude, "pickup_latitude"
    )
    assert_field_value(
        actual.pickup_longitude, expected.pickup_longitude, "pickup_longitude"
    )
    assert_field_value(
        actual.dropoff_address, expected.dropoff_address, "dropoff_address"
    )
    ...
    assert_field_value(
        actual.route_distance_meters,
        expected.route_distance_meters,
        "route_distance_meters",
    )
    assert_field_value(
        actual.route_duration_seconds,
        expected.route_duration_seconds,
    ...
    assert_field_value(
        ...
        else expected.order_delivery_status,
        "order_delivery_status",
    )
...
def assert_create_order_delivery_response(
    *,
    ...
    assert_field_value(
        actual.order_delivery_status, "selected", "order_delivery_status"
    )
...
def assert_get_delivery_response(
    *,
    ...
    assert_field_value(
        actual.external_delivery_id,
        expected.external_delivery_id,
...
def _assert_fake_transition(
    *,
    ...
    _assert_delivery_entity(
        actual=actual, expected=expected, status=status, order_delivery_status=status
    )
    assert_field_value(actual.synced, True, "synced")
