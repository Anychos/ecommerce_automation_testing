...
def _activate_delivery(
    *, private_payments_client: PaymentsAPIClient, create_payment: PaymentFixture
) -> None:
    ...
    assert_succeed_payment_response(
        actual=response_data, expected=create_payment.response
    )
...
class TestDeliveryPositive:
    ...
    def test_create_delivery_success(
        self, private_delivery_client: DeliveriesAPIClient, create_order: OrderFixture
    ) -> None:
        ...
        response_data = CreateOrderDeliveryResponseSchema.model_validate_json(
            response.text
        )
        assert_create_order_delivery_response(
            actual=response_data, expected=request, order_id=order_id
        )
        assert_json_schema(
    ...
    def test_get_delivery_info_success(
        ...
        response_data = GetDeliveryByIdResponseResponseSchema.model_validate_json(
            response.text
        )
        assert_get_delivery_response(
            actual=response_data, expected=create_delivery.response
        )
        assert_json_schema(
            actual=response.json(), schema=response_data.model_json_schema()
        )
    ...
    def test_fake_assign_delivery_success(
        ...
        response_data = FakeAssignDeliveryResponseSchema.model_validate_json(
            response.text
        )
        assert_assigned_delivery(
            actual=response_data, expected=create_payment.delivery.response
        )
        assert_json_schema(
            actual=response.json(), schema=response_data.model_json_schema()
        )
    ...
    def test_fake_pickup_delivery_success(
        ...
        assign_response_data = FakeAssignDeliveryResponseSchema.model_validate_json(
            assign_response.text
        )
        assert_assigned_delivery(
            actual=assign_response_data, expected=create_payment.delivery.response
        )
        assert_json_schema(
            actual=assign_response.json(),
            schema=assign_response_data.model_json_schema(),
        )
        ...
        response_data = FakePickupDeliveryResponseSchema.model_validate_json(
            response.text
        )
    ...
    def test_fake_deliver_delivery_success(
        ...
        pickup_response_data = FakePickupDeliveryResponseSchema.model_validate_json(
            pickup_response.text
        )
        assert_picked_delivery(
            actual=pickup_response_data, expected=assign_response_data
        )
        assert_json_schema(
            actual=pickup_response.json(),
            schema=pickup_response_data.model_json_schema(),
        )
        ...
        response_data = FakeDeliverDeliveryResponseSchema.model_validate_json(
            response.text
        )
    ...
    def test_fake_cancel_delivery_success(
        ...
        response_data = FakeCancelDeliveryResponseSchema.model_validate_json(
            response.text
        )
