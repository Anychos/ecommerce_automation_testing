...
if TYPE_CHECKING:
    from src.api.fixtures.user import DeletedUserFixture, UserFixture
...
from src.api.tools.assertions.authentication import (
    assert_already_registered_email_response,
    assert_invalid_email_format_response,
    assert_login_response,
    assert_register_response,
    assert_wrong_login_data_response,
)
...
class TestAuthenticationPositive:
    ...
    def test_user_registration(self, auth_client: AuthenticationAPIClient) -> None:
    ...
    def test_user_login(
        self, auth_client: AuthenticationAPIClient, user: UserFixture
    ) -> None:
    ...
    @pytest.mark.smoke
    @allure.story(Story.ADMIN_LOGIN)
    @allure.severity(Severity.BLOCKER)
    @allure.title("Логин админа с валидными данными")
    def test_admin_login(
        self, auth_client: AuthenticationAPIClient, admin: UserFixture
    ) -> None:
...
class TestAuthenticationNegative:
    ...
    def test_user_registration_registered_email(
        self, auth_client: AuthenticationAPIClient, user: UserFixture
    ) -> None:
    ...
    @allure.severity(Severity.NORMAL)
    @allure.title(
        "Логин пользователя с валидным, но не зарегистрированным email и валидным паролем"
    )
    def test_user_login_unregistered_email(
        self, auth_client: AuthenticationAPIClient, user: UserFixture
    ) -> None:
    ...
    @pytest.mark.parametrize(
        "email", ["test@mail.", "test@.ru", "@mail.ru", "", "testmail.ru"]
    )
    @allure.title("Логин пользователя с невалидным форматом email и валидным паролем")
    def test_user_login_invalid_email_format(
        self, auth_client: AuthenticationAPIClient, email: str, user: UserFixture
    ) -> None:
        request = LoginRequestSchema.model_construct(
    ...
    @allure.severity(Severity.NORMAL)
    @allure.title(
        "Логин пользователя с зарегистрированным email и неподходящим паролем"
    )
    def test_user_login_inappropriate_password(
        self, auth_client: AuthenticationAPIClient, user: UserFixture
    ) -> None:
    ...
    @allure.story(Story.USER_LOGIN)
    @allure.severity(Severity.NORMAL)
    @allure.title("Логин удаленного пользователя")
    def test_deleted_user_login_error(
        self, auth_client: AuthenticationAPIClient, delete_user: DeletedUserFixture
    ) -> None:
        request = LoginRequestSchema(
            email=delete_user.email, password=delete_user.password
        )
    ...
    @allure.story(Story.USER_LOGIN)
    @allure.severity(Severity.NORMAL)
    @allure.title("Логин пользователя с просроченным токеном")
    def test_user_login_expired_token(
        self, auth_client: AuthenticationAPIClient, delete_user: DeletedUserFixture
    ) -> None:
        request = LoginRequestSchema(
            email=delete_user.email, password=delete_user.password
