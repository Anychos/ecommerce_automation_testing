...
from src.api.tools.assertions.product import (
    assert_create_product_response,
    assert_delete_product_response,
    assert_empty_required_field_response,
    assert_full_update_product_response,
    assert_get_product_response,
    assert_get_products_response,
    assert_invalid_image_url_response,
    assert_partial_update_product_response,
...
class TestProductPositive:
    ...
    def test_create_product(
        self, admin_private_product_client: ProductAPIClient
    ) -> None:
    ...
    def test_get_product(
        ...
        response = user_private_product_client.get_product_api(
            product_id=create_available_product.product_id
        )
        assert_status_code(response.status_code, HTTPStatus.OK)

        response_data = GetProductResponseSchema.model_validate_json(response.text)
        assert_get_product_response(
            actual=response_data, expected=create_available_product.response
        )
        assert_json_schema(
            actual=response.json(), schema=response_data.model_json_schema()
        )
    ...
    def test_get_products(
        ...
        response_data = TypeAdapter(GetProductsResponseSchema).validate_json(
            response.text
        )
        assert_get_products_response(
    ...
    def test_full_update_product(
        ...
        response = admin_private_product_client.full_update_product_api(
            product_id=create_available_product.product_id, request=request
        )
        assert_status_code(response.status_code, HTTPStatus.OK)
    ...
    def test_partial_update_product(
        ...
        response = admin_private_product_client.partial_update_product_api(
            product_id=create_available_product.product_id, request=request
        )
        assert_status_code(response.status_code, HTTPStatus.OK)
    ...
    def test_delete_product(
        ...
        response = admin_private_product_client.delete_product_api(
            product_id=create_available_product.product_id
        )
        assert_status_code(response.status_code, HTTPStatus.OK)
...
class TestProductNegative:
    ...
    def test_create_product_wrong_data_format(
        ...
        response_data = InputValidationErrorResponseSchema.model_validate_json(
            response.text
        )
        assert_wrong_data_format_response(
            actual=response_data, wrong_field=wrong_field, wrong_value=wrong_value
        )
        assert_json_schema(
            actual=response.json(), schema=response_data.model_json_schema()
        )
    ...
    @allure.severity(Severity.NORMAL)
    @pytest.mark.parametrize(
        "name, description, price, image_url, wrong_field, wrong_value",
        [
            (
                "",
                fake_ru.description(),
    ...
    def test_create_product_empty_required_field(
        self,
        admin_private_product_client: ProductAPIClient,
        name,
        description,
    ...
    @pytest.mark.parametrize(
        "image_url", ["example.com/image.jpg", "https://example.com/image.gif"]
    )
    @allure.feature(Feature.ADMIN_PRODUCTS)
    @allure.story(Story.ADMIN_CREATE_PRODUCT)
    @allure.severity(Severity.NORMAL)
    @allure.title("Создание продукта с некорректным URL изображения")
    def test_create_product_with_wrong_image_url(
        self, admin_private_product_client: ProductAPIClient, image_url: str
    ) -> None:
